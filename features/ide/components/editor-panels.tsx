...
import { ArrowUpRight, CheckCircle2, ExternalLink, GitBranch, Mail, Rocket, Terminal } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { experience, initLines, projects, skillGroups } from "../data/workspace";
...
export function EditorPanel({
  ...
  return (
    <motion.div key={activeFile} {...panelMotion} className="min-h-full">
      {activeFile === "about" && <AboutPanel readyStep={readyStep} onOpen={onOpenFile} />}
      {activeFile === "projects" && <ProjectsPanel activeProject={activeProject} onSelectProject={onSelectProject} />}
      {activeFile === "experience" && <ExperiencePanel expandedCommit={expandedCommit} onExpand={onExpandCommit} />}
...
function AboutPanel({ readyStep, onOpen }: { readyStep: number; onOpen: (file: FileId) => void }) {
  ...
  return (
    <section className="editor-grid">
      <CodeWindow title="terminal" className="min-h-[22rem]">
        <div className="grid gap-3 p-4 font-mono text-sm text-muted-foreground">
          {initLines.slice(0, Math.min(readyStep, initLines.length)).map((line) => (
            ...
            >
              <span className="text-primary">{line === "Ready" ? "ok" : "$"}</span> {line}
            </motion.p>
          ))}
          {!isReady && (
      ...
      <article className={cn("ide-card grid content-center gap-5 p-6 sm:p-8", isReady ? "opacity-100" : "opacity-45")}>
        <div className="space-y-3">
          <p className="ide-eyebrow">Senior Frontend Engineer</p>
          <h1 className="max-w-3xl text-balance text-[clamp(2.2rem,7vw,5rem)] font-semibold leading-[0.98]">
            Siavash builds polished product interfaces with engineering discipline.
          </h1>
          <p className="max-w-2xl text-pretty text-base leading-7 text-muted-foreground">
            I design and implement modern frontend systems where interaction, accessibility, architecture, and product clarity
            work together.
          </p>
        </div>
  ...
  return (
    <section className="editor-grid editor-grid-projects">
      <div className="ide-card overflow-hidden">
        <div className="border-b border-border px-4 py-3 font-mono text-xs uppercase text-muted-foreground">Git History</div>
        <div className="relative grid gap-1 p-3 before:absolute before:bottom-6 before:left-7 before:top-6 before:w-px before:bg-border">
          {projects.map((project) => (
            <button
              key={project.id}
              className={cn("project-commit", activeProject.id === project.id && "is-active")}
              onClick={() => onSelectProject(project)}
            >
              <span className="commit-dot" />
              <span className="min-w-0">
                <strong className="block truncate text-foreground">{project.name}</strong>
                <small className="block text-pretty text-muted-foreground">{project.summary}</small>
              </span>
            </button>
          ))}
        </div>
      ...
      <article className="ide-card grid gap-5 p-4 sm:p-5">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold">{activeProject.name}.tsx</h2>
        </div>
        <DefinitionList
          items={[
            ...
            <div className="grid gap-1">
              {branch.children.map(([name, years, description], index) => (
                <div className="dependency-row" key={name}>
                  <span className="font-mono text-xs text-primary">{index === branch.children.length - 1 ? "`--" : "|--"}</span>
    ...
    <section className="grid max-w-5xl gap-3">
      {experience.map((item, index) => (
        <article className="ide-card overflow-hidden" key={item.company}>
          ...
          >
            <span className="font-mono text-xs text-primary">commit {String(index + 1).padStart(2, "0")}</span>
      ...
      <div className="grid gap-1 p-4 font-mono text-sm">
        ...
        {rows.map(([key, value]) => (
          <div className="inspect-row" key={key}>
            ...
            <strong className="font-normal text-muted-foreground">&quot;{value}&quot;,</strong>
          </div>
        ))}
    ...
    <section className="ide-card grid max-w-5xl gap-5 p-5 md:grid-cols-[minmax(0,0.9fr)_minmax(18rem,1fr)]">
      <div>
        ...
        <p className="mt-3 text-sm leading-7 text-muted-foreground">
          Email, GitHub, LinkedIn, and resume actions are presented like developer commands.
        </p>
      </div>
...
function CodeWindow({ children, className, title }: { children: ReactNode; className?: string; title: string }) {
  return (
...
function TechRow({ items }: { items: string[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((tech) => (
        <span className="rounded-full border border-border bg-muted px-2.5 py-1 text-xs text-muted-foreground" key={tech}>
          {tech}
        </span>
      ))}
    </div>
...
function ActionLink({ children, href, icon }: { children: ReactNode; href: string; icon: ReactNode }) {
  return (
